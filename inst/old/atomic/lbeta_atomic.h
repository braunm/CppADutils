#ifndef __LBETA_ATOMIC
#define __LBETA_ATOMIC
...
namespace CppAD {
  namespace Atomic{
    ...
    class atomic_lbeta : public CppAD::atomic_base<double> {
       
    public:
      atomic_lbeta(const std::string& name) :
	CppAD::atomic_base<double>(name) {}
      
    private:
      
      template<typename TX>
      bool lbeta_deriv(const MatrixBase<TX>& x,
			double& f)
	...
	if ((q <= 0) && (p == 0)) {
	  lbeta_deriv(x.row(0),f);
	  y(0) = f;      
	}
	
	if ((q <= 1) && (p == 1)) {
	  VectorXd df(n);
	  lbeta_deriv(x.row(0),f,df);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1));    
	}
    ...
    AScalar lbeta(const AScalar& a, const AScalar& b) {
      ...
      static atomic_lbeta func("atomic_lbeta");
