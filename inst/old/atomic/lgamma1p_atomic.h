#ifndef __LGAMMA1P_ATOMIC
#define __LGAMMA1P_ATOMIC
...
namespace CppAD {  
  namespace Atomic{
    ...
    class atomic_lgamma1p : public CppAD::atomic_base<double> {
       
    public:
      atomic_lgamma1p(const std::string& name) :
	CppAD::atomic_base<double>(name) {}
      
    private:
      
      template<typename TX>
      bool lgamma1p_deriv(const MatrixBase<TX>& x,
			double& f)
	...
	if ((q <= 0) && (p == 0)) {
	  lgamma1p_deriv(x.row(0),f);
	  y(0) = f;      
	}
	
	if ((q <= 1) && (p == 1)) {
	  VectorXd df(n);
	  lgamma1p_deriv(x.row(0),f,df);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1));    
	}
	...
	if ((q <= 2) && (p == 2)) {
	  VectorXd df(n);
	  MatrixXd hess(n,n);
	  lgamma1p_deriv(x.row(0),f,df,hess);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1)); 	
    ...
    AScalar lgamma1p(const AScalar& a) {
      ...
      static atomic_lgamma1p func("atomic_lgamma1p");
