#ifndef __INVLOGIT_ATOMIC
#define __INVLOGIT_ATOMIC
...
namespace CppAD {  
  namespace Atomic{
    ...
    class atomic_invlogit : public CppAD::atomic_base<double> {
       
    public:
      atomic_invlogit(const std::string& name) :
	CppAD::atomic_base<double>(name) {}
      
    private:

      template<typename TX>
      bool invlogit_deriv(const MatrixBase<TX>& x,
			double& f)
	...
	if ((q <= 0) && (p == 0)) {
	  invlogit_deriv(x.row(0),f);
	  y(0) = f;      
	}
	
	if ((q <= 1) && (p == 1)) {
	  VectorXd df(n);
	  invlogit_deriv(x.row(0),f,df);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1));    
	}
	...
	if ((q <= 2) && (p == 2)) {
	  VectorXd df(n);
	  MatrixXd hess(n,n);
	  invlogit_deriv(x.row(0),f,df,hess);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1)); 	
    ...
    AScalar invlogit(const AScalar& a) {
      ...
      static atomic_invlogit func("atomic_invlogit");
