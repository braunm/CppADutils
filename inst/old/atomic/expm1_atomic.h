#ifndef __EXPM1_ATOMIC
#define __EXPM1_ATOMIC
...
namespace CppAD {
  namespace Atomic{
    ...
    class atomic_expm1 : public CppAD::atomic_base<double> {
       
    public:
    atomic_expm1(const std::string& name) :
      CppAD::atomic_base<double>(name) {}
      
    private:
      
      template<typename TX>
	bool expm1_deriv(const MatrixBase<TX>& x,
			 double& f){
	...
	if ((q <= 0) && (p == 0)) {
	  expm1_deriv(x.row(0),f);
	  y(0) = f;      
	}
	
	if ((q <= 1) && (p == 1)) {
	  VectorXd df(n);
	  expm1_deriv(x.row(0),f,df);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1));    
	}
    ...
    AScalar expm1(const AScalar& a) {
      ...
      static atomic_expm1 func("atomic_expm1");
