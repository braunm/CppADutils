#ifndef __LOG1PEXP_ATOMIC
#define __LOG1PEXP_ATOMIC
...
namespace CppAD {
  namespace Atomic{
    ...
    class atomic_log1pexp : public CppAD::atomic_base<double> {
      
    public:
      atomic_log1pexp(const std::string& name) :
	CppAD::atomic_base<double>(name) {}
      
    private:
      
      template<typename TX>
      bool log1pexp_deriv(const MatrixBase<TX>& x,
			double& f)
	...
	if ((q <= 0) && (p == 0)) {
	  log1pexp_deriv(x.row(0),f);
	  y(0) = f;      
	}
	
	if ((q <= 1) && (p == 1)) {
	  VectorXd df(n);
	  log1pexp_deriv(x.row(0),f,df);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1));    
	}
	...
	if ((q <= 2) && (p == 2)) {
	  VectorXd df(n);
	  MatrixXd hess(n,n);
	  log1pexp_deriv(x.row(0),f,df,hess);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1)); 	
    ...
    AScalar log1pexp(const AScalar& a) {
      ...
      static atomic_log1pexp func("atomic_log1pexp");
