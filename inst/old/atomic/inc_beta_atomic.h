#ifndef __INC_BETA_ATOMIC
#define __INC_BETA_ATOMIC
...
namespace CppAD { 
  namespace Atomic{
    ...
    class atomic_inc_beta : public CppAD::atomic_base<double> {
       
    public:
      atomic_inc_beta(const std::string& name) :
	CppAD::atomic_base<double>(name) {}
      
    private:
      
      template<typename TX>
      bool inc_beta_deriv(const MatrixBase<TX>& x,
			double& f)
	...
	if ((q <= 1) && ((p == 1) || (p == 2))) {
	  ...
	  MatrixXd hess(n,n);
	  inc_beta_deriv(x.row(0),f,df, hess);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1));    
	  if (p==2) {
    ...
    AScalar inc_beta(const AScalar& z, const AScalar& a, const AScalar& b) {
      
      static atomic_inc_beta func("atomic_inc_beta");
