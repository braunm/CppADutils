#ifndef __LGAMMA_ATOMIC
#define __LGAMMA_ATOMIC
...
namespace CppAD {  
  namespace Atomic{
    ...
    class atomic_lgamma : public CppAD::atomic_base<double> {
       
    public:
      atomic_lgamma(const std::string& name) :
	CppAD::atomic_base<double>(name) {}
      
    private:
      
      template<typename TX>
      bool lgamma_deriv(const MatrixBase<TX>& x,
			double& f)
	...
	f = R::lgammafn(a);
	const double psi_a = R::digamma(a);
	...
	f = R::lgammafn(a);
	const double psi_a = R::digamma(a);
	const double psi_1_a = R::trigamma(a);
	...
	if ((q <= 0) && (p == 0)) {
	  lgamma_deriv(x.row(0),f);
	  y(0) = f;      
	}
	
	if ((q <= 1) && (p == 1)) {
	  VectorXd df(n);
	  lgamma_deriv(x.row(0),f,df);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1));    
	}
	...
	if ((q <= 2) && (p == 2)) {
	  VectorXd df(n);
	  MatrixXd hess(n,n);
	  lgamma_deriv(x.row(0),f,df,hess);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1)); 	
    ...
    AScalar lgamma(const AScalar& a) {
      ...
      static atomic_lgamma func("atomic_lgamma");
