#ifndef __LOGINVLOGIT_ATOMIC
#define __LOGINVLOGIT_ATOMIC
...
namespace CppAD {  
  namespace Atomic{
    ...
    class atomic_loginvlogit : public CppAD::atomic_base<double> {
       
    public:
      atomic_loginvlogit(const std::string& name) :
	CppAD::atomic_base<double>(name) {}
      
    private:

      template<typename TX>
      bool loginvlogit_deriv(const MatrixBase<TX>& x,
			double& f)
	...
	if ((q <= 0) && (p == 0)) {
	  loginvlogit_deriv(x.row(0),f);
	  y(0) = f;      
	}
	
	if ((q <= 1) && (p == 1)) {
	  VectorXd df(n);
	  loginvlogit_deriv(x.row(0),f,df);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1));    
	}
	...
	if ((q <= 2) && (p == 2)) {
	  VectorXd df(n);
	  MatrixXd hess(n,n);
	  loginvlogit_deriv(x.row(0),f,df,hess);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1)); 	
    ...
    AScalar loginvlogit(const AScalar& a) {
      ...
      static atomic_loginvlogit func("atomic_loginvlogit");
