#ifndef __LOG1P_ATOMIC
#define __LOG1P_ATOMIC
...
namespace CppAD {
  namespace Atomic{
    ...
    class atomic_log1p : public CppAD::atomic_base<double> {
       
    public:
    atomic_log1p(const std::string& name) :
      CppAD::atomic_base<double>(name) {}
      
    private:
      
      template<typename TX>
	bool log1p_deriv(const MatrixBase<TX>& x,
			 double& f){
	...
	if ((q <= 0) && (p == 0)) {
	  log1p_deriv(x.row(0),f);
	  y(0) = f;      
	}
	
	if ((q <= 1) && (p == 1)) {
	  VectorXd df(n);
	  log1p_deriv(x.row(0),f,df);
	  y(0) = f;      
	  y(1) = df.dot(x.row(1));    
	}
    ...
    AScalar log1p(const AScalar& a) {
      ...
      static atomic_log1p func("atomic_log1p");
