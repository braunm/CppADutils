...
test_that("dbeta_log",{
    ...
    R_func <- function(x) {
        ...
        res <- 0
        for (i in 1:n) {
            res <- res + dbeta(x[3*(i-1)+1], x[3*(i-1)+2], x[3*i], log=TRUE)
        }
        return(res)
    }

    x <- c(0.8, 2.3, 4.4, 0.3, 2.0, 2.1, .9, 3, 3)
...
context("dlogitbeta_log")

test_that("dlogitbeta_log",{
    require(numDeriv)
    fn <- function(z, a, b) {
        p <- exp(z)/(1+exp(z))
        res <- dbeta(p,a,b,log=TRUE) + log(p) + log(1-p)
        return(res)
    }
